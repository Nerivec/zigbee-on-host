...
import {
    ...
    ZigbeeSecurityLevel,
    encodeZigbeeSecurityHeader,
    makeKeyedHash,
    makeKeyedHashByType,
    registerDefaultHashedKeys,
} from "../zigbee/zigbee.js";
import { encodeCoordinatorDescriptors } from "./endpoints.js";
...
export type ConfigurationAttributes = {
    /**
     * NOTE: Pre-encoded as "sendable" ZDO response (see endpoints.ts for more details):
     */
    address: Buffer;
    /**
     * 05-3474-23 #2.3.2.3
...
export class OTRCPDriver extends EventEmitter<AdapterDriverEventMap> {
    ...
    public async onFrame(buffer: Buffer): Promise<void> {
        ...
        logger.debug(() => `<--- SPINEL[tid=${spinelFrame.header.tid} cmdId=${spinelFrame.commandId} length=${spinelFrame.payload.byteLength}]`, NS);
    ...
    public async onStreamRawFrame(payload: Buffer, metadata: SpinelStreamRawMetadata | undefined): Promise<void> {
        ...
        try {
            ...
            if (macFCF.frameType !== MACFrameType.CMD && macFCF.frameType !== MACFrameType.DATA) {
                logger.debug(() => "<-~- MAC Ignoring frame with type not CMD/DATA", NS);
                return;
            }
    ...
    public async sendCommand(commandId: SpinelCommandId, buffer: Buffer, waitForResponse = true, timeout = 10000): Promise<SpinelFrame | undefined> {
        ...
        logger.debug(() => `---> SPINEL[tid=${tid} cmdId=${commandId} length=${buffer.byteLength} wait=${waitForResponse} timeout=${timeout}]`, NS);
        const spinelFrame = {
            header: {
                tid,
                nli: 0,
    ...
    public async sendMACFrame(seqNum: number, payload: Buffer, dest16: number | undefined, dest64: bigint | undefined): Promise<void> {
        const func = async (): Promise<void> => {
            try {
                ...
                logger.error(`=x=> MAC[seqNum=${seqNum} dest16=${dest16} dest64=${dest64}] ${(error as Error).message}`, NS);
            }
        };
    ...
    public async sendMACCommand(
        ...
        logger.debug(() => `===> MAC CMD[seqNum=${macSeqNum} cmdId=${cmdId} dest16=${dest16} dest64=${dest64} extSource=${extSource}]`, NS);
    ...
    public async processMACCommandFrame(data: Buffer, macHeader: MACHeader): Promise<void> {
        ...
        logger.debug(() => `<=== MAC CMD[cmdId=${macHeader.commandId} macSource=${macHeader.source16}:${macHeader.source64}]`, NS);
    ...
    public processMACAssocRsp(data: Buffer, offset: number, _macHeader: MACHeader): number {
        ...
        logger.debug(() => `<=== MAC ASSOC_RSP[address=${address} status=${MACAssociationStatus[status]}]`, NS);

        return offset;
    }

    public async sendMACAssocRsp(dest64: bigint, newNetwork16: number, status: MACAssociationStatus | number): Promise<void> {
        logger.debug(() => `===> MAC ASSOC_RSP[dest64=${dest64} newNetwork16=${newNetwork16} status=${status}]`, NS);

        const finalPayload = Buffer.alloc(3);
        let offset = 0;
        finalPayload.writeUInt16LE(newNetwork16, offset);
        offset += 2;
    ...
    public async sendZigbeeNWKCommand(
        ...
        logger.debug(
            () =>
                `===> NWK CMD[seqNum=(${nwkSeqNum}/${macSeqNum}) cmdId=${cmdId} macDest16=${macDest16} nwkSource16=${nwkSource16} nwkDest16=${nwkDest16} nwkDest64=${nwkDest64} nwkRadius=${nwkRadius}]`,
            NS,
        );
    ...
    public async processZigbeeNWKCommandFrame(data: Buffer, macHeader: MACHeader, nwkHeader: ZigbeeNWKHeader): Promise<void> {
        ...
        logger.debug(
            () =>
                `<=== NWK CMD[cmdId=${cmdId} macSource=${macHeader.source16}:${macHeader.source64} nwkSource=${nwkHeader.source16}:${nwkHeader.source64}]`,
            NS,
        );
    ...
    public async processZigbeeNWKRouteReq(data: Buffer, offset: number, macHeader: MACHeader, nwkHeader: ZigbeeNWKHeader): Promise<number> {
        ...
        logger.debug(
            () => `<=== NWK ROUTE_REQ[id=${id} dst=${destination16} dst64=${destination64} pathCost=${pathCost} manyToOne=${manyToOne}]`,
            NS,
        );
    ...
    public async sendZigbeeNWKRouteReq(manyToOne: ZigbeeNWKManyToOne, destination16: number, destination64?: bigint): Promise<void> {
        logger.debug(() => `===> NWK ROUTE_REQ[manyToOne=${manyToOne} destination16=${destination16} destination64=${destination64}]`, NS);
        ...
        await this.sendZigbeeNWKCommand(
            ...
            finalPayload,
            ZigbeeMACConsts.BCAST_ADDR, // macDest16
            true, // nwkSecurity
    ...
    public processZigbeeNWKRouteReply(data: Buffer, offset: number, _macHeader: MACHeader, _nwkHeader: ZigbeeNWKHeader): number {
        ...
        logger.debug(
            () =>
                `<=== NWK ROUTE_REPLY[id=${id} orig=${originator16} orig64=${originator64} resp=${responder16} resp64=${responder64} pathCost=${pathCost}]`,
            NS,
        );
        // TODO
    ...
    public async sendZigbeeNWKRouteReply(
        ...
        logger.debug(
            () =>
                `===> NWK ROUTE_REPLY[requestDest1stHop16=${requestDest1stHop16} requestRadius=${requestRadius} requestId=${requestId} originator16=${originator16} responder16=${responder16} originator64=${originator64} responder64=${responder64}]`,
            NS,
        );
     ...
     * 05-3474-R #3.4.3
     */
    public processZigbeeNWKStatus(data: Buffer, offset: number, _macHeader: MACHeader, _nwkHeader: ZigbeeNWKHeader): number {
        const status = data.readUInt8(offset);
        offset += 1;
        ...
        logger.debug(() => `<=== NWK NWK_STATUS[status=${ZigbeeNWKStatus[status]} dst=${destination}]`, NS);
        // TODO
     ...
     */
    public async sendZigbeeNWKStatus(
        requestDest1stHop16: number,
        ...
        logger.debug(
            () =>
                `===> NWK NWK_STATUS[requestDest1stHop16=${requestDest1stHop16} requestSource16=${requestSource16} status=${status} destination=${destination}]`,
            NS,
        );
        ...
        await this.sendZigbeeNWKCommand(
            ...
            requestDest1stHop16 === ZigbeeMACConsts.BCAST_ADDR ? undefined : this.address16ToAddress64.get(requestDest1stHop16), // nwkDest64
     ...
     */
    public async sendZigbeeNWKLeave(neighbor16: number, destination16: number, rejoin: boolean): Promise<void> {
        logger.debug(() => `===> NWK LEAVE[neighbor16=${neighbor16} destination16=${destination16} rejoin=${rejoin}]`, NS);
        ...
        await this.sendZigbeeNWKCommand(
            ...
            finalPayload,
            destination16 < ZigbeeConsts.BCAST_MIN ? neighbor16 : ZigbeeMACConsts.BCAST_ADDR, // macDest16
            true, // nwkSecurity
            ZigbeeConsts.COORDINATOR_ADDRESS, // nwkSource16
            destination16, // nwkDest16
    ...
    public async processZigbeeNWKRejoinReq(data: Buffer, offset: number, _macHeader: MACHeader, nwkHeader: ZigbeeNWKHeader): Promise<number> {
        ...
        logger.debug(() => `<=== NWK REJOIN_REQ[capabilities=${capabilities}]`, NS);
    ...
    public processZigbeeNWKRejoinResp(data: Buffer, offset: number, _macHeader: MACHeader, _nwkHeader: ZigbeeNWKHeader): number {
        ...
        if (status !== MACAssociationStatus.SUCCESS) {
            logger.error(`<=x= NWK REJOIN_RESP[newAddress=${newAddress} status=${MACAssociationStatus[status]}]`, NS);
        } else {
            logger.debug(() => `<=== NWK REJOIN_RESP[newAddress=${newAddress}]`, NS);
        }
     ...
     */
    public async sendZigbeeNWKRejoinResp(requestSource16: number, newNetwork16: number, status: MACAssociationStatus | number): Promise<void> {
        logger.debug(() => `===> NWK REJOIN_RESP[requestSource16=${requestSource16} newNetwork16=${newNetwork16} status=${status}]`, NS);

        const finalPayload = Buffer.from([ZigbeeNWKCommandId.REJOIN_RESP, newNetwork16 & 0xff, (newNetwork16 >> 8) & 0xff, status]);

        await this.sendZigbeeNWKCommand(
            ZigbeeNWKCommandId.REJOIN_RESP,
            finalPayload,
            requestSource16, // macDest16
    ...
    public processZigbeeNWKReport(data: Buffer, offset: number, _macHeader: MACHeader, _nwkHeader: ZigbeeNWKHeader): number {
        ...
        logger.debug(() => `<=== NWK NWK_REPORT[extendedPANId=${extendedPANId} reportType=${reportType} conflictPANIds=${conflictPANIds}]`, NS);
    ...
    public processZigbeeNWKUpdate(data: Buffer, offset: number, _macHeader: MACHeader, _nwkHeader: ZigbeeNWKHeader): number {
        ...
        logger.debug(() => `<=== NWK NWK_UPDATE[extendedPANId=${extendedPANId} updateId=${updateId} updateType=${updateType} panIds=${panIds}]`, NS);
        // TODO
    ...
    public async processZigbeeNWKEdTimeoutRequest(data: Buffer, offset: number, _macHeader: MACHeader, nwkHeader: ZigbeeNWKHeader): Promise<number> {
        ...
        logger.debug(() => `<=== NWK ED_TIMEOUT_REQUEST[requestedTimeout=${requestedTimeout} configuration=${configuration}]`, NS);
    ...
    public async sendZigbeeNWKEdTimeoutResponse(requestDest16: number, requestedTimeout: number): Promise<void> {
        logger.debug(() => `===> NWK ED_TIMEOUT_RESPONSE[requestDest16=${requestDest16} requestedTimeout=${requestedTimeout}]`, NS);
        ...
        await this.sendZigbeeNWKCommand(
            ...
            finalPayload,
            requestDest16, // macDest16
            true, // nwkSecurity
            ZigbeeConsts.COORDINATOR_ADDRESS, // nwkSource16
            requestDest16, // nwkDest16
            this.address16ToAddress64.get(requestDest16), // nwkDest64 TODO: can `undefined`?
            1, // nwkRadius
        );
    }
    ...
    public async processZigbeeNWKCommissioningRequest(
        ...
        logger.debug(() => `<=== NWK COMMISSIONING_REQUEST[assocType=${assocType} capabilities=${capabilities}]`, NS);
        ...
        await this.sendZigbeeNWKCommissioningResponse(nwkHeader.source16!, newNetwork16, status);
        ...
        if (status === MACAssociationStatus.SUCCESS) {
            ...
            await this.sendZigbeeAPSTransportKeyNWK(
                ...
                this.netParams.networkKey,
                this.netParams.networkKeySequenceNumber,
                this.address16ToAddress64.get(newNetwork16)!, // valid from `associate`
            );
        }
    ...
    public processZigbeeNWKCommissioningResponse(data: Buffer, offset: number, _macHeader: MACHeader, _nwkHeader: ZigbeeNWKHeader): number {
        ...
        if (status !== MACAssociationStatus.SUCCESS) {
            logger.error(
                `<=x= NWK COMMISSIONING_RESPONSE[newAddress=${newAddress} status=${MACAssociationStatus[status] ?? "NWK_ADDR_CONFLICT"}]`,
                NS,
            );
        } else {
            logger.debug(() => `<=== NWK COMMISSIONING_RESPONSE[newAddress=${newAddress}]`, NS);
        }
     ...
     *
     * @param requestSource16
     * @param newNetwork16 the new 16-bit network address assigned, may be same as `requestDest16`
     */
    public async sendZigbeeNWKCommissioningResponse(
        ...
        logger.debug(() => `===> NWK COMMISSIONING_RESPONSE[requestSource16=${requestSource16} newNetwork16=${newNetwork16} status=${status}]`, NS);

        const finalPayload = Buffer.from([ZigbeeNWKCommandId.COMMISSIONING_RESPONSE, newNetwork16 & 0xff, (newNetwork16 >> 8) & 0xff, status]);
    ...
    public async sendZigbeeAPSCommand(
        ...
        logger.debug(
            () =>
                `===> APS CMD[seqNum=(${apsCounter}/${nwkSeqNum}/${macSeqNum}) cmdId=${cmdId} macDest=${macDest16} nwkDest=${nwkDest16}:${nwkDest64} nwkDiscRoute=${nwkDiscoverRoute} nwkSec=${nwkSecurity} apsDelivery=${apsDeliveryMode} apsSec=${apsSecurityHeader !== undefined}]`,
            NS,
        );
    ...
    public async sendZigbeeAPSData(
        ...
        logger.debug(
            () =>
                `===> APS DATA[seqNum=(${apsCounter}/${nwkSeqNum}/${macSeqNum}) macDest16=${macDest16} nwkDiscoverRoute=${nwkDiscoverRoute} nwkDest16=${nwkDest16} nwkDest64=${nwkDest64} apsDeliveryMode=${apsDeliveryMode}]`,
            NS,
        );
    ...
    public async onZigbeeAPSACKRequest(macHeader: MACHeader, nwkHeader: ZigbeeNWKHeader, apsHeader: ZigbeeAPSHeader): Promise<void> {
        logger.debug(
            () =>
                `===> APS ACK[dest=${nwkHeader.source16} seqNum=${nwkHeader.seqNum} destEp=${apsHeader.sourceEndpoint} clusterId=${apsHeader.clusterId}]`,
            NS,
        );
        ...
        const ackMACFrame = encodeMACFrameZigbee(
            {
                ...
                // sourcePANId: undefined, // panIdCompression=true
                source16: macHeader.destination16,
                fcs: 0,
            },
            ackNWKFrame,
        );
        ...
        switch (apsHeader.frameControl.frameType) {
            ...
            case ZigbeeAPSFrameType.INTERPAN: {
                ...
                if (apsHeader.profileId === ZigbeeConsts.ZDO_PROFILE_ID) {
                    processed = await this.filterZDO(data, apsHeader.clusterId!, macHeader.source16!, nwkHeader.source16, nwkHeader.source64);
                }
    ...
    public async processZigbeeAPSCommandFrame(
        ...
        logger.debug(
            () =>
                `<=== APS CMD[cmdId=${cmdId} macSource=${macHeader.source16}:${macHeader.source64} nwkSource=${nwkHeader.source16}:${nwkHeader.source64}]`,
            NS,
        );
        ...
        switch (cmdId) {
            ...
            case ZigbeeAPSCommandId.UPDATE_DEVICE: {
                offset = this.processZigbeeAPSUpdateDevice(data, offset, macHeader, nwkHeader, apsHeader);
                break;
            }
    ...
    public processZigbeeAPSTransportKey(
        ...
        switch (keyType) {
            ...
            case ZigbeeAPSConsts.CMD_KEY_HIGH_SEC_NWK: {
                ...
                logger.debug(
                    () => `<=== APS TRANSPORT_KEY[keyType=${keyType} key=${key} seqNum=${seqNum} destination=${destination} source=${source}]`,
                    NS,
                );
            ...
            case ZigbeeAPSConsts.CMD_KEY_TC_LINK: {
                ...
                logger.debug(() => `<=== APS TRANSPORT_KEY[keyType=${keyType} key=${key} destination=${destination} source=${source}]`, NS);
                break;
            }
            ...
            case ZigbeeAPSConsts.CMD_KEY_APP_LINK: {
                ...
                logger.debug(() => `<=== APS TRANSPORT_KEY[keyType=${keyType} key=${key} partner=${partner} initiatorFlag=${initiatorFlag}]`, NS);
                break;
            }
        }
     ...
     */
    public async sendZigbeeAPSTransportKeyTC(macDest16: number, nwkDest16: number, key: Buffer, destination64: bigint): Promise<void> {
        ...
        logger.debug(() => `===> APS TRANSPORT_KEY_TC[key=${key.toString("hex")} destination64=${destination64}]`, NS);
     ...
     */
    public async sendZigbeeAPSTransportKeyNWK(
        macDest16: number,
        nwkDest16: number,
        key: Buffer,
        seqNum: number,
        ...
        // TODO: tunneling support `, tunnelDest?: bigint`
        logger.debug(() => `===> APS TRANSPORT_KEY_NWK[key=${key.toString("hex")} seqNum=${seqNum} destination64=${destination64}]`, NS);
     ...
     */
    public async sendZigbeeAPSTransportKeyAPP(
        macDest16: number,
        nwkDest16: number,
        key: Buffer,
        partner: bigint,
        ...
        // TODO: tunneling support `, tunnelDest?: bigint`
        logger.debug(() => `===> APS TRANSPORT_KEY_APP[key=${key.toString("hex")} partner=${partner} initiatorFlag=${initiatorFlag}]`, NS);
     ...
     * 05-3474-R #4.4.11.2
     */
    public processZigbeeAPSUpdateDevice(
        data: Buffer,
        offset: number,
        _macHeader: MACHeader,
        _nwkHeader: ZigbeeNWKHeader,
    ...
    public processZigbeeAPSRemoveDevice(
        ...
        logger.debug(() => `<=== APS REMOVE_DEVICE[target=${target}]`, NS);
     ...
     * @param target64
     */
    public async sendZigbeeAPSRemoveDevice(macDest16: number, nwkDest16: number, target64: bigint): Promise<void> {
    ...
    public async processZigbeeAPSRequestKey(
        ...
        if (device64 !== undefined) {
            ...
            if (keyType === ZigbeeAPSConsts.CMD_KEY_APP_MASTER) {
                ...
                logger.debug(() => `<=== APS REQUEST_KEY[keyType=${keyType} partner=${partner}]`, NS);
                ...
                logger.debug(() => `<=== APS REQUEST_KEY[keyType=${keyType}]`, NS);

                if (this.trustCenterPolicies.allowTCKeyRequest === TrustCenterKeyRequestPolicy.ALLOWED) {
                    await this.sendZigbeeAPSTransportKeyTC(macHeader.source16!, nwkHeader.source16!, this.netParams.tcKey, device64);
                }
            ...
            logger.warning(`Received key request from unknown device source=${nwkHeader.source16}`, NS);
        }
     ...
     */
    public async sendZigbeeAPSRequestKey(macDest16: number, nwkDest16: number, keyType: 0x02, partner64: bigint): Promise<void>;
    public async sendZigbeeAPSRequestKey(macDest16: number, nwkDest16: number, keyType: 0x04): Promise<void>;
    public async sendZigbeeAPSRequestKey(macDest16: number, nwkDest16: number, keyType: 0x02 | 0x04, partner64?: bigint): Promise<void> {
        logger.debug(() => `===> APS REQUEST_KEY[keyType=${keyType} partner64=${partner64}]`, NS);
     ...
     */
    public async sendZigbeeAPSSwitchKey(macDest16: number, nwkDest16: number, seqNum: number): Promise<void> {
        logger.debug(() => `===> APS SWITCH_KEY[seqNum=${seqNum}]`, NS);
    ...
    public processZigbeeAPSTunnel(
        ...
        logger.debug(() => `<=== APS TUNNEL[destination=${destination} tunneledAPSFrame=${tunneledAPSFrame}]`, NS);
     ...
     * @param tAPSHeader
     * @param tApsCmdFrame SHALL be the APS command frame to be sent to the destination
     * @param tAPSMIC
     */
    public async sendZigbeeAPSTunnel(
        macDest16: number,
        nwkDest16: number,
        destination64: bigint,
        tApsCmdFrame: Buffer,
        ...
        const finalPayload = Buffer.alloc(28 + tApsCmdFrame.byteLength);
        let offset = 0;
        ...
        offset += 8;
        // TODO ??
        offset = encodeZigbeeAPSHeader(finalPayload, offset, {
            frameControl: {
                frameType: ZigbeeAPSFrameType.DATA,
                deliveryMode: ZigbeeAPSDeliveryMode.UNICAST,
                ackFormat: false,
        ...
        // SHALL indicate that a link key was used and SHALL include the extended nonce field.
        offset = encodeZigbeeSecurityHeader(finalPayload, offset, {
            control: {
                level: ZigbeeSecurityLevel.NONE,
                keyId: ZigbeeKeyType.LINK,
                nonce: true,
            },
            frameCounter: 0, // TODO: ??
            source64: this.netParams.eui64,
        ...
        finalPayload.set(tAPSMIC, offset);
        offset += 4;
    ...
    public async processZigbeeAPSVerifyKey(
        ...
        if (macHeader.source16 !== ZigbeeMACConsts.BCAST_ADDR) {
            logger.debug(() => `<=== APS VERIFY_KEY[keyType=${keyType} source=${source} keyHash=${keyHash.toString("hex")}]`, NS);
            ...
            if (keyType === ZigbeeAPSConsts.CMD_KEY_TC_LINK) {
                ...
                await this.sendZigbeeAPSConfirmKey(macHeader.source16!, nwkHeader.source16!, status, keyType, source);
            } else if (keyType === ZigbeeAPSConsts.CMD_KEY_APP_MASTER) {
                // this is illegal for TC
                await this.sendZigbeeAPSConfirmKey(macHeader.source16!, nwkHeader.source16!, 0xa3 /* ILLEGAL_REQUEST */, keyType, source);
            } else {
                await this.sendZigbeeAPSConfirmKey(macHeader.source16!, nwkHeader.source16!, 0xaa /* NOT_SUPPORTED */, keyType, source);
            }
        }
     ...
     */
    public async sendZigbeeAPSVerifyKey(macDest16: number, nwkDest16: number, keyType: number, source64: bigint, hash: Buffer): Promise<void> {
        logger.debug(() => `===> APS VERIFY_KEY[keyType=${keyType} source64=${source64} hash=${hash.toString("hex")}]`, NS);
    ...
    public processZigbeeAPSConfirmKey(
        ...
        logger.debug(() => `<=== APS CONFIRM_KEY[status=${status} keyType=${keyType} destination=${destination}]`, NS);
     ...
     */
    public async sendZigbeeAPSConfirmKey(
        macDest16: number,
        nwkDest16: number,
        status: number,
        keyType: number,
        ...
        logger.debug(() => `===> APS CONFIRM_KEY[status=${status} keyType=${keyType} destination64=${destination64}]`, NS);
    ...
    public processZigbeeAPSRelayMessageDownstream(
        ...
        logger.debug(() => `<=== APS RELAY_MESSAGE_DOWNSTREAM[destination64=${destination64}]`, NS);
    ...
    public processZigbeeAPSRelayMessageUpstream(
        ...
        logger.debug(() => `<=== APS RELAY_MESSAGE_UPSTREAM[source64=${source64}]`, NS);
            ...
            if (assocType === 0x00) {
                ...
                if (this.trustCenterPolicies.allowJoins) {
                    if (source16 === undefined) {
                        ...
                        newNetwork16 = this.assignNetworkAddress();

                        if (newNetwork16 === 0xffff) {
    ...
    private async filterZDO(
        ...
        finalPayload[0] = this.nextZDOSeqNum();

        logger.debug(
            () =>
                `===> COORD_ZDO[seqNum=${finalPayload[0]} clusterId=${clusterId} macDest16=${macDest16} nwkDest16=${nwkDest16} nwkDest64=${nwkDest64}]`,
            NS,
        );
    ...
    /**
     * Format is
     * - network data: 1024 bytes
     * - device count: 2 bytes
     * - device data: 512 bytes * (device count)
     */
    public async saveState(): Promise<void> {
        const state = Buffer.alloc(1024 + 2 + this.deviceTable.size * 512);
        let offset = 0;
        ...
        state.writeUInt32LE(this.netParams.networkKeyFrameCounter + 1024, offset); // TODO: deal with jumping
        offset += 4;
        state.writeUInt8(this.netParams.networkKeySequenceNumber, offset);
        offset += 1;
        state.set(this.netParams.tcKey, offset);
        offset += ZigbeeConsts.SEC_KEYSIZE;
        state.writeUInt32LE(this.netParams.tcKeyFrameCounter + 1024, offset); // TODO: deal with jumping
        offset += 4;

        // reserved
        offset += 1024 - offset; // currently: 962
    ...
    public async sendZDO(payload: Buffer, nwkDest16: number, nwkDest64: bigint | undefined, clusterId: number): Promise<number> {
        ...
        const macDest16 = nwkDest16 < ZigbeeConsts.BCAST_MIN ? nwkDest16 : ZigbeeMACConsts.BCAST_ADDR; // TODO: from routing table

        logger.debug(() => `===> ZDO[seqNum=${payload[0]} clusterId=${clusterId} macDest16=${macDest16} nwkDest=${nwkDest16}:${nwkDest64}]`, NS);
        ...
        return await this.sendZigbeeAPSData(
            ...
            nwkDest16, // nwkDest16
            nwkDest64, // nwkDest64
            macDest16 === ZigbeeMACConsts.BCAST_ADDR ? ZigbeeAPSDeliveryMode.BCAST : ZigbeeAPSDeliveryMode.UNICAST, // apsDeliveryMode
            clusterId, // clusterId
            ZigbeeConsts.ZDO_PROFILE_ID, // profileId
            ZigbeeConsts.ZDO_ENDPOINT, // destEndpoint
            ZigbeeConsts.ZDO_ENDPOINT, // sourceEndpoint
    ...
    public async sendUnicast(
        ...
        const macDest16 = dest16; // TODO: from routing
