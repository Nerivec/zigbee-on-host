    ...
    if (macPayload.byteLength > 0) {
        ...
        if (protocolVersion === 3 /* ZigbeeNWKConsts.VERSION_GREEN_POWER */) {
            ...
            if (nwkFCF.security && nwkHeader.securityHeader) {
                ...
                if (!accepted) {
                    logger_js_1.logger.warning(() => `<-x- NWK Rejecting replay frame src16=${nwkHeader.source16}:${resolvedSource64} counter=${nwkHeader.securityHeader?.frameCounter}`, NS);
                    return;
                }
            }
            if (nwkFCF.frameType === 0 /* ZigbeeNWKFrameType.DATA */) {
                ...
                if (nwkHeader.source16 === undefined && nwkHeader.source64 === undefined) {
                    logger_js_1.logger.debug(() => `<-~- APS Ignoring frame with no sender info seqNum=${nwkHeader.seqNum}`, NS);
                    return;
                }
                ...
                if (apsHeader.frameControl.frameType !== 2 /* ZigbeeAPSFrameType.ACK */ && apsHandler.isDuplicateFrame(nwkHeader, apsHeader)) {
                    logger_js_1.logger.debug(() => `<=~= APS Ignoring duplicate frame seqNum=${nwkHeader.seqNum} counter=${apsHeader.counter}`, NS);
                    return;
                }
