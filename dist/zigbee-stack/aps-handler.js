...
class APSHandler {
    ...
    async #sendDataInternal(params, apsCounter, attempt) {
        ...
        if (isFragment) {
            ...
            const fragmentation = fragmentInfo.isFirst
                ? 1 /* ZigbeeAPSFragmentation.FIRST */ : fragmentInfo.isLast
                ? 3 /* ZigbeeAPSFragmentation.LAST */ : 2 /* ZigbeeAPSFragmentation.MIDDLE */;
    ...
    async sendTransportKeyTC(nwkDest16, key, destination64) {
        ...
        const finalPayload = Buffer.allocUnsafe(18 + 16 /* ZigbeeAPSConsts.CMD_KEY_LENGTH */);
        let offset = 0;
        ...
        true, // nwkSecurity
        nwkDest16, // nwkDest16
        undefined, 0 /* ZigbeeAPSDeliveryMode.UNICAST */, // apsDeliveryMode
        {
            control: {
                level: 0 /* ZigbeeSecurityLevel.NONE */,
    ...
    async sendTransportKeyNWK(nwkDest16, key, seqNum, destination64) {
        ...
        const finalPayload = Buffer.allocUnsafe(19 + 16 /* ZigbeeAPSConsts.CMD_KEY_LENGTH */);
        let offset = 0;
        offset = finalPayload.writeUInt8(5 /* ZigbeeAPSCommandId.TRANSPORT_KEY */, offset);
    ...
    async sendTransportKeyAPP(nwkDest16, key, partner, initiatorFlag) {
        ...
        const finalPayload = Buffer.allocUnsafe(11 + 16 /* ZigbeeAPSConsts.CMD_KEY_LENGTH */);
        let offset = 0;
        offset = finalPayload.writeUInt8(5 /* ZigbeeAPSCommandId.TRANSPORT_KEY */, offset);
    ...
    async processUpdateDevice(data, offset, macHeader, nwkHeader, _apsHeader) {
        ...
        if (status === 0 /* ZigbeeAPSUpdateDeviceStatus.STANDARD_DEVICE_SECURED_REJOIN */) {
            ...
            false, true);
            this.#updateSourceRouteForChild(device16, nwkHeader.source16, nwkHeader.source64);
            const tApsCmdPayload = Buffer.allocUnsafe(19 + 16 /* ZigbeeAPSConsts.CMD_KEY_LENGTH */);
            let tunnelOffset = 0;
     ...
     * DEVICE SCOPE: Trust Center
     */
    async sendRelayMessageDownstream(nwkDest16) {
        logger_js_1.logger.debug(() => `===> APS RELAY_MESSAGE_DOWNSTREAM[${"TODO"}]`, NS);
        const finalPayload = Buffer.allocUnsafe(1);
        let offset = 0;
        offset = finalPayload.writeUInt8(17 /* ZigbeeAPSCommandId.RELAY_MESSAGE_DOWNSTREAM */, offset);
        // TODO
        const result = await this.sendCommand(17 /* ZigbeeAPSCommandId.RELAY_MESSAGE_DOWNSTREAM */, finalPayload, 0 /* ZigbeeNWKRouteDiscovery.SUPPRESS */, // nwkDiscoverRoute
        false, // nwkSecurity
        nwkDest16, // nwkDest16
        undefined, 0 /* ZigbeeAPSDeliveryMode.UNICAST */, // apsDeliveryMode
        undefined, // apsSecurityHeader
    ...
    getLQITableResponse(startIndex) {
        ...
        for (const [addr64, entry] of this.#context.deviceTable) {
            if (entry.neighbor) {
                ...
                const deviceTypeByte = (deviceType & 0x03) | ((rxOnWhenIdle << 2) & 0x0c) | ((affinity << 4) & 0x70) | ((0 /* reserved */ << 7) & 0x80);
    ...
    async respondToCoordinatorZDORequest(data, clusterId, nwkDest16, nwkDest64) {
        ...
        if (finalPayload) {
            ...
            try {
                ...
                nwkDest16, // nwkDest16
                nwkDest64, 0 /* ZigbeeAPSDeliveryMode.UNICAST */, // apsDeliveryMode
                clusterId | 0x8000, 0 /* ZigbeeConsts.ZDO_PROFILE_ID */, 0 /* ZigbeeConsts.ZDO_ENDPOINT */, 0 /* ZigbeeConsts.ZDO_ENDPOINT */, // sourceEndpoint
                undefined);
            }
            catch {
                // logged in `sendData`
