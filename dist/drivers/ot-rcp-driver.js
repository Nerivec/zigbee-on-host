...
class OTRCPDriver extends node_events_1.default {
    ...
    findBestSourceRoute(destination16, destination64) {
        ...
        if (validEntries.length > 1) {
            validEntries.sort((a, b) => {
                ...
                const stalenessPenaltyA = ageA > CONFIG_NWK_ROUTE_STALENESS_TIME
                    ? Math.min(2, (ageA - CONFIG_NWK_ROUTE_STALENESS_TIME) / CONFIG_NWK_ROUTE_STALENESS_TIME)
                    : 0;
                const stalenessPenaltyB = ageB > CONFIG_NWK_ROUTE_STALENESS_TIME
    ...
    async saveState() {
        ...
        await (0, promises_1.writeFile)(this.savePath, state.subarray(0, offset));
    }
    /**
